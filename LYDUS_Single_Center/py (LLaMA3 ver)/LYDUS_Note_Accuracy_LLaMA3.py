import os, re, yaml, argparse, ast, time, threading
import pandas as pd
import numpy as np
...
class LocalLlamaClient:
    ...
    def chat(self, model: str, messages: list, temperature: float = 0, max_tokens: int = 800, top_p: float = 1.0):
        # OpenAI 메시지 포맷과 유사하게 받아 chat template 사용
...
def _run_clinical(quiq: pd.DataFrame, llama_client: LocalLlamaClient, model: str) :
    client = llama_client
    df = quiq.copy()
    ...
    def api_call(row, value_col='Value', results=[]):
        report = "<value note> " + str(row[value_col])
        try:
...
def _run_radiology(quiq: pd.DataFrame, llama_client: LocalLlamaClient, model: str):
    client = llama_client
    df = quiq.copy()
    ...
    def check_error1(row):
        try:
            ...
            if 'error 1' in row_dict:
                return 0 if str(row_dict['error 1']).lower() != 'no error' else 100
            return None
...
def get_unstructured_accuracy(quiq: pd.DataFrame, model: str, api_key: str) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    # OpenAI 대신 LocalLlamaClient 사용 (api_key는 호환용으로만 유지)
    llama = LocalLlamaClient(model_path=model, dtype="bfloat16")

    df_clinical, mean_clinical, std_clinical = _run_clinical(quiq, llama, model)
    df_radiology, mean_radiology, std_radiology = _run_radiology(quiq, llama, model)
    ...
    # ← 원본 오타 수정: 따옴표 누락을 보정하고, 집계 로직/열 이름은 동일 유지
...
if __name__ == '__main__':
    ...
    with open(os.path.join(save_path, 'note_accuracy_total.txt'), 'w', encoding = 'utf-8') as file :
