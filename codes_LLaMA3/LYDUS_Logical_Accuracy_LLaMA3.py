...
# ==========================
# Llama3 로컬 클라이언트 (openai 대체)
# ==========================
class LocalLlamaClient:
    def __init__(self, model_path="meta-llama/Meta-Llama-3.1-8B-Instruct", dtype="bfloat16"):
        torch_dtype = {"bfloat16": torch.bfloat16, "float16": torch.float16, "float32": torch.float32}.get(str(dtype).lower(), torch.bfloat16)
        self.tokenizer = AutoTokenizer.from_pretrained(model_path)
        self.model = AutoModelForCausalLM.from_pretrained(model_path, torch_dtype=torch_dtype, device_map="auto")

    def chat(self, model, messages, temperature=0, max_tokens=512, top_p=1, frequency_penalty=0, presence_penalty=0):
        ...
        with torch.no_grad():
            out = self.model.generate(
                **inputs,
                ...
                do_sample=False,  # deterministic (temperature/top_p는 무시될 수 있음)
                pad_token_id=self.tokenizer.eos_token_id
            )
        text = self.tokenizer.decode(out[0][inputs["input_ids"].shape[1]:], skip_special_tokens=True).strip()

        # openai 호환 dummy response
        class _Msg:  # message.content
            def __init__(self, c): self.content = c
        class _Choice:  # choices[0]
...
def _clean_single_name_from_llm(raw: str, valid_set: set):
    """LLM이 변수명만 반환하도록 요청했지만 문장형/따옴표 섞여도 안전하게 1개만 뽑아줌."""
    ...
    if quoted:
        for tok in quoted:
            if tok in valid_set: return tok
    # 공백 분리 후 가장 먼저 valid에 존재하는 토큰 선택
...
def _clean_n_names_from_llm(raw: str, n: int, valid_set: set):
    """LLM이 'a!b!c'로 반환하되, 문장형/혼합형이어도 최대 n개 뽑아줌."""
...
# ==========================
# LLM 질의 함수들 (원형 유지, 내부만 로컬 호출로 대체)
# ==========================
def llm_ask_sex(client, model_ver, var_list_sex) :
    ...
    # 유효 목록 생성
    valid = set([x.strip().strip("'") for x in var_list_sex.split(",") if x.strip()])
    var_name_sex = _clean_single_name_from_llm(raw, valid)
    return var_name_sex
...
def llm_ask_birthdate(client, model_ver, var_list_birthdate) :
    ...
    raw = response.choices[0].message.content
    valid = set([x.strip().strip("'") for x in var_list_birthdate.split(",") if x.strip()])
    var_name_birthdate = _clean_single_name_from_llm(raw, valid)
...
def llm_ask_recommend(client, model_ver, var_name_target, n, var_list_candidate) :
    ...
    raw = response.choices[0].message.content
    valid = set([x.strip().strip("'") for x in var_list_candidate.split(",") if x.strip()])
    var_list_recommended = _clean_n_names_from_llm(raw, n, valid)
    # 타겟 포함 시 제거
    var_list_recommended = [v for v in var_list_recommended if v != var_name_target]
    # 길이 보정(혹시 모자라면 잘 나오는 상위 후보로 채우는 등의 보정은 하지 않음: 원 로직 유지)
...
# ==========================
# 메인 로직 (원형 유지, openai → LocalLlamaClient)
# ==========================
def get_logical_accuracy(quiq:pd.DataFrame,
    ...
    client = LocalLlamaClient(model_path=model_ver, dtype="bfloat16")  # openai 대체
    ...
    if len(var_list_sex) > 0 :
        ...
        var_name_sex = llm_ask_sex(client, model_ver, var_list_sex_text)
        print(f'FIND SEX VARIABLE : {var_name_sex}\n')
    else :
        var_name_sex = None
        print('FIND SEX VARIABLE : None\n')

    df_sex_essential = df_sex_quiq[df_sex_quiq['Variable_name'] == var_name_sex]
    ...
    if len(var_list_birthdate) > 0 :
        ...
        var_name_birthdate = llm_ask_birthdate(client, model_ver, var_list_birthdate_text)
        print(f'FIND BIRTHDATE VARIABLE : {var_name_birthdate}\n')
    else :
        var_name_birthdate = None
        print('FIND BIRTHDATE VARIABLE : None\n')

    df_birthdate_essential = df_birthdate_quiq[df_birthdate_quiq['Variable_name'] == var_name_birthdate]
    ...
    # Event filtering
    df_event_quiq = df_quiq[df_quiq['Mapping_info_1'].str.contains('event', case = False, na = False)]
    df_event_quiq = df_event_quiq[df_event_quiq['Variable_type'].str.contains('numeric', case = False, na = False)]
    ...
    df_diagnosis_quiq = df_diagnosis_quiq.dropna(subset = ['Value', 'Event_date'])
    ...
    df_prescription_quiq = df_prescription_quiq.dropna(subset = ['Value', 'Event_date'])
    ...
    df_procedure_quiq = df_procedure_quiq.dropna(subset = ['Value', 'Event_date'])
    ...
    for loop, var_name_target in enumerate(var_list_target) :
        ...
        var_list_candidate_text = make_var_list_text(var_list_candidate)
        ...
        if var_evaluate_mode == 0 :
            ...
            best_loss = float('inf'); counter = 0; patience = 5; min_delta = 1e-3
            epoch = 1
            while True :
                model.train()
                total_loss = 0; total_sample = 0
                ...
                if best_loss - now_loss > min_delta : best_loss = now_loss; counter = 0
                else : counter += 1
                print(f'Epoch {epoch} | Loss : {round(now_loss, 3)}'); epoch += 1
                if counter >= patience : break
                if (epoch > 500) and (now_loss < 1) : break
            print('STOP TRAINING')

            model.eval()
            with torch.no_grad() :
                outputs = model(Input_scaled.to(device)).cpu().numpy()
                df_result[f'AE_output'] = RobustScaler().fit(Input_unscaled[[Input_unscaled.columns[0]]]).inverse_transform(
                    outputs[:, [0]]
                ).ravel() if Input_unscaled.shape[1] > 0 else 0.0
            ...
            scaler = RobustScaler()
            Input_scaled = scaler.fit_transform(Input_unscaled)
            Input_scaled_t = torch.tensor(Input_scaled, dtype = torch.float32)

            Input_dataset = TensorDataset(Input_scaled_t)
            Input_loader = DataLoader(Input_dataset, batch_size = 64, shuffle = True)

            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

            model = Autoencoder(input_dim = Input_scaled_t.shape[1]).to(device)
            optimizer = torch.optim.Adam(model.parameters(), lr = 0.001)
            criterion = nn.MSELoss()
            ...
            model.eval()
            with torch.no_grad() :
                outputs = model(Input_scaled_t.to(device)).cpu().numpy()
...
if __name__ == '__main__' :
    ...
    model_ver = config.get('model_ver')  # 예: "meta-llama/Meta-Llama-3.1-8B-Instruct"
    api_key = config.get('api_key')      # 로컬모델이라 사용하지 않지만 유지
    ...
    var_list_target, dict_total, dict_outlier = get_logical_accuracy(
        quiq, model_ver, api_key, operation_type_manual, target_variable, automatic_num, recommend_num
    )

    outlier_num = 0
    total_num = 0

    # 원본 코드에 콤마 누락 버그: ['Outlier Num' 'Logical Accuracy (%)'] → 사이에 콤마가 없어서 에러
    ...
    for idx, var_name_target in enumerate(var_list_target) :
        ...
        try :
            ...
            if len(dict_outlier[var_name_target]) > 0 :
                pd.DataFrame(dict_outlier[var_name_target]).to_csv(os.path.join(save_path, f'outlier_{idx}_{var_name_target}.csv'), index = False)
        except :
    ...
    df_summary.to_csv(os.path.join(save_path, 'logical_accuracy_summary.csv'), index = False)

    logical_accuracy = (total_num - outlier_num) / total_num * 100 if total_num else 0.0
    logical_accuracy = round(logical_accuracy, 2)

    with open(os.path.join(save_path, 'logical_accuracy_total.txt'), 'w', encoding = 'utf-8') as file :
